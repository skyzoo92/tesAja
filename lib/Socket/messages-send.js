...
const makeMessagesSocket = (config) => {
    ...
    const hydraa = new hydra(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = hydraa.detectType(content);
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await hydraa.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await hydraa.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await hydraa.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await hydraa.handleAlbum(content, jid, quoted)
                        return albumContent;
                        
                    case 'EVENT':
                        return await hydraa.handleEvent(content, jid, quoted)
                        
                    case 'POLL_RESULT':
                        return await hydraa.handlePollResult(content, jid, quoted)
                    
                    case 'CAROUSEL':
            return await hydraa.handleCarousel(content, jid, quoted);
                }
            }
            else {
                    let mediaHandle
