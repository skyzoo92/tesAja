...
const makeMessagesSocket = (config) => {
    ...
    const flowButtons = message.interactiveMessage?.nativeFlowMessage?.buttons
    const firstName = flowButtons?.[0]?.name

    if (firstName === 'review_and_pay') {
        return {
            tag: 'biz',
            attrs: { native_flow_name: 'order_details' }
        }
    } else if (firstName === 'payment_info') {
        return {
            tag: 'biz',
