...
const makeMessagesSocket = (config) => {
    ...
    const rahmi = new riookk(Utils_1, waUploadToServer, relayMessage);
    ...
    const carouselCards = await Promise.all(
        cards.map(async (card) => {
            // Cek apakah ini mode product atau image biasa
            ...
            viewOnceMessage: {
                message: {
                    interactiveMessage: WAProto_1.proto.Message.InteractiveMessage.create({
                        body: WAProto_1.proto.Message.InteractiveMessage.Body.create({ text: caption }),
                        footer: WAProto_1.proto.Message.InteractiveMessage.Footer.create({ text: footer }),
